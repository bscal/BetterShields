...
@Mixin(PlayerEntity.class) public abstract class PlayerEntityMixin extends LivingEntity implements
		PlayerEntityAccessor
{

	@Unique
	private int m_lastOffhandAttackedTicks;
		...
		if (target.isAttackable()) {
			if (!target.handleAttack(this)) {
				...
				if (target instanceof LivingEntity) {
					h = EnchantmentHelper.getAttackDamage(this.getOffHandStack(), ((LivingEntity)target).getGroup());
				} else {
	...
	public float GetOffhandAttackCooldownProgress(float baseTime) {
		return MathHelper.clamp(((float)m_lastOffhandAttackedTicks + baseTime) / this.getAttackCooldownProgressPerTick(), 0.0F, 1.0F);
	}
}
